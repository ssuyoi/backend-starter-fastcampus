...
public class CacheConfiguration {
    ...
    public RedisTemplate<String, UserEntity> userEntityRedisTemplate(
        ...
        // StringRedisSerializer() 문자열만 처리, 주로 key에 사용
        template.setKeySerializer(new StringRedisSerializer());
        // JacksonJsonRedisSerializer<>() JSON 형태로 저장
    ...
    public RedisTemplate<String, List<CrashSession>> crashSessionsListRedisTemplate(
        ...
        JavaType type = objectMapper.getTypeFactory()
            .constructCollectionType(List.class, CrashSession.class);

        template.setValueSerializer(
            new Jackson2JsonRedisSerializer<CrashSession>(objectMapper, type)
        );
        return template;
    }
}
